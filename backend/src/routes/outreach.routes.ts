...
const getFromEmail = () => process.env.BREVO_SENDER_EMAIL || process.env.GMAIL_USER || "hello@hirecompass.com";
const getFromName = () => process.env.BREVO_SENDER_NAME || "HireCompass";

const sendBrevoEmail = async (payload: { to: string; subject: string; text: string; attachments?: { filename: string; content: string }[] }) => {
  ...
  const response = await fetch("https://api.brevo.com/v3/smtp/email", {
    ...
    body: JSON.stringify({
      ...
      htmlContent: payload.text.replace(/\n/g, "<br>"),
...
router.post("/send", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (!process.env.BREVO_API_KEY) {
      return res.status(503).json({ error: "BREVO_API_KEY not configured. Add it to .env to enable email sending via Brevo." });
    }
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      try {
        let emailBody = record.finalEmail || record.generatedEmail || "";
        let subject = record.finalSubject || record.emailSubject || `Internship Inquiry – ${record.companyName}`;
        const toEmail = record.recruiterEmail;

        if (!toEmail || !emailBody) throw new Error("Missing recipient email or email body");

        // Clean up unwanted tags generated by LLM
        const extractTag = (text: string, tag: string) => {
          const match = text.match(new RegExp(`<${tag}>([\\s\\S]*?)<\\/${tag}>`, 'i'));
          return match ? match[1].trim() : text;
        };
        emailBody = extractTag(emailBody, 'EMAIL').replace(/<EMAIL>|<\/EMAIL>/ig, "").trim();
        subject = extractTag(subject, 'SUBJECT').replace(/<SUBJECT>|<\/SUBJECT>/ig, "").trim();

        const payload: any = { to: toEmail, subject, text: emailBody };
        if (attachmentData) payload.attachments = [{ filename: attachmentData.filename, content: attachmentData.content }];

        const info = await sendBrevoEmail(payload);
...
Return JSON: { "subject": "...", "body": "..." }`;
    ...
    const payload: any = { to: record.recruiterEmail, subject: generated.subject || `Following up – ${record.companyName}`, text: generated.body };
    if (attachmentData) payload.attachments = [{ filename: attachmentData.filename, content: attachmentData.content }];
